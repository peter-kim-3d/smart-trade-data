...
- 견고성: 일부 데이터센터에서 1688/CDN 이 HTTP/2 스트림을 리셋(`curl 92`, 2B 응답)하거나 연결 실패(`HTTP:000`)하면 **자동으로 `--http1.1` 재시도** — 데스크톱 프로브 + 모바일 **페이지·상세페이지·이미지(메인/상세/커버)·동영상** 다운로드 전부(즉 모든 네트워크 요청). H2 리셋은 http_code 200 을 찍고도 파일을 자르므로 curl 종료코드·파일크기까지 검사해 깨진 파일을 저장하지 않는다. 데스크톱 경로가 프로브 통과 후에도 실패하면 모바일로 최종 폴백.
