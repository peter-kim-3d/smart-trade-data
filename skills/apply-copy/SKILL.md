...
5. 각 결과를 view로 확인: 한글이 깨끗한지(깨짐/오탈자/누락), 지정한 카피가 정확히 들어갔는지,
   불필요한 추가 문구 없는지, 제품 크게 안 망가졌는지. 문제면 1회 재시도한다.
   - 허용된 재시도 후에도 한글 글리프만 틀렸다면, 승인된 clean 이미지를 기준으로 정확한 카피를
     결정론적으로 합성할 수 있다. ImageMagick이 없어도 `/usr/bin/ffmpeg`의 `drawtext`와
     한국어 지원 폰트(예: `/usr/share/fonts/opentype/unifont/unifont.otf`)를 사용한다.
   - 잘못 생성된 문구 위에 덧쓰지 말고 clean 이미지에서 다시 시작하며, 원문과 같은 의미 영역에
     정확한 카피만 배치한다. 합성 결과를 다시 view로 확인하고 보고서의 생성 요청·재시도·결정론
     보정 수를 각각 보존한다.
   - ffmpeg/한국어 폰트도 없거나 보정 결과가 불량하면 파일명과 결함을 명시해 보고한다.
